...
import { buildCjsAliasMap, createCellixAzureFunctionsRolldownConfig, prepareCellixAzureFunctionsDeploy } from './index.js';
...
describe('@cellix/config-rolldown', () => {
	...
	it('creates the standard Cellix Azure Functions rolldown config', async () => {
		...
		expect(config.external).toEqual(expect.arrayContaining([expect.any(RegExp), '@azure/functions-core']));
	...
	it('prepares the deploy artifact for Azure Functions', async () => {
		...
		const deployPackageJson = JSON.parse(await fs.readFile(path.join(appDir, 'deploy/package.json'), 'utf8')) as Record<string, unknown>;
	...
	it('fails prepare:deploy when the bundle entry is missing', async () => {
		...
		await expect(prepareCellixAzureFunctionsDeploy({ appDir })).rejects.toThrow('Run the app build before prepare:deploy.');
	});
});
...
async function cleanupTempRepos(): Promise<void> {
	...
	await Promise.all(entries.filter((entry) => entry.isDirectory() && entry.name.startsWith('cellix-config-rolldown-')).map((entry) => fs.rm(path.join(tempRootDir, entry.name), { recursive: true, force: true })));
}
